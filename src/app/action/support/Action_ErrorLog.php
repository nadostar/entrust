...
* 
*/
class Action_ErrorLog extends _Action_Support {
	
	protected $menu_allow_no = MenuData::LOG_ERROR_REPORT_ID;
	...
	protected function doAction() {
		...
		switch ($this->action) {
			...
			default:
				...
				$this->output->setTmpl('support/errorlog.php');		
				break;
		}	
	}
	...
	private function search() {
		...
		$data = Logic_Log::getErrorLogDataLimited($this->log_db, $pager->limit(), $pager->offset());
		...
		$this->output->setTmpl('support/_errorlog_list.php');
	}

	private function viewer() {
		$id = $this->getQuery('id');

		$log = Logic_Log::getErrorLogDataById($this->log_db, $id);

		$this->output->assign('log', $log);
		$this->output->setTmpl('support/_errorlog_viewer.php');
	}
}