...
class Action_Survey extends _Action_Api {
	...
	protected function doAction() {
		...
		if('true' == $testmode) {
			LogManager::debug("==== test mode ====");
			$this->testmodeProc($snapshot, $params);
		} else {
			LogManager::debug("==== live mode ====");
	...
	protected function testmodeProc($snapshot, $params) {
		$project = Logic_Live::findProjectById($this->slave_db, $snapshot['pid']);
		$partner = $snapshot['extra']['partner'];
		...
		if($history === false) {
			...
			Logic_Live::writeTest($this->master_db, $historydata);
